...
concepts = ["jazz", "classical", "hip hop", "EDM"]
audio_path_temp = "seed_sampling_genre/seed_{}/A music track.wav"
save_file = "filtered_seeds_genre.json"
...
for concept in concepts:
    ...
    filtered_seeds = []
    # if concept == "violin":
    #     threshold = 0.06  # violin seems to have generally higher similarity, so use a slightly higher threshold
    # else:
    #     threshold = 0.05
    for seed, _ in data[concept]:
        ...
        if similarity.item() < threshold:  # arbitrary threshold for "negative" seeds
            count_filtered += 1
            filtered_seeds.append((seed, similarity.item()))

    selected_seeds[concept] = filtered_seeds
    print(f"Total seeds tested for concept '{concept}': {len(data[concept])}")
    print(f"Number of seeds with similarity < 0.06 for concept '{concept}': {count_filtered}")

# Save selected seeds to a new JSON file
with open(save_file, "w") as f:
    json.dump(selected_seeds, f, indent=4)